...
def VGG16(include_top=True, weights='imagenet',
    ...
    if weights == 'imagenet':
        ...
        fcLayer = Dense(4096, name='fc')(lastCnnLayer)
        classificationLayer = Dense(2, activation='linear')(fcLayer)
        model_with_new_last_layer = Model(input=model.input, output=classificationLayer)
        
    return model_with_new_last_layer
...
if __name__ == '__main__':
    ...
    #for single_train_pos_img in glob.glob('data/train/positive/*.jpg'):    
        ...
        train_img = image.load_img(train_img_path, target_size=(224, 224)) #appears to resize the image correctly: https://github.com/fchollet/keras/blob/master/keras/preprocessing/image.py
        train_x.append(np.asarray(train_img))        
        train_y.append(0)        #not holding leash = 0
    ...
    model.fit(np.array(train_x),np.array(train_y),nb_epoch=1, batch_size=20)


    for single_test_img in glob.glob('data/test/positive/*.jpg'):    
        test_img_path = single_test_img
        ...
        '''
        for elems in decode_predictions(preds):
            pred_class = ''
            class_score = 0
            for elem in elems:
                if elem[2] > class_score:
                    pred_class = elem[1]
                    class_score = elem[2]

            pred_count = class_dict.get(pred_class,0)
            class_dict[pred_class] = pred_count + 1
            print(pred_class)
        
    sorted_class_dict = sorted(class_dict.items(), key=operator.itemgetter(1))
