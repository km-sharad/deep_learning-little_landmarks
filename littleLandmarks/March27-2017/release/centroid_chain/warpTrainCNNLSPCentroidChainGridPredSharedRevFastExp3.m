function warpTrainCNNLSPCentroidChainGridPredSharedRevFastExp3(CONFIG)
...
% PROC_DIR = [CONFIG.procRoot 'struct_cnn_pose_sgd/LSP/cnn/'];
...
imdb = data.lsp_anno_oc;

% Determine the training images.
is_train = [imdb.istrain];
is_train = find(is_train > 0);
is_test = [imdb.istest];
is_test = find(is_test > 0);

exp_id = 'e_cnn_centroid_chain_lsp_11_rev_fast_exp_adam_17_demo';
...
opts.target_joint_name = joint_names(target_inds);
...
opts.add_random_rotation_jitter = true;
opts.rotation_jitter_range = [-80 80]; % Angle in degrees
...
opts.lr_flip_function = @lrFlipDataRecord;
opts.norm_height_function = @getTorsoHeight;
...
opts.img_root = CONFIG.lspDataDir;
...
opts.train.val = is_test(1:100);

% Remove the training images with wrong annotatiosn.
opts.train.train = setdiff(opts.train.train, getLSPWrongAnnoTrainImages());
